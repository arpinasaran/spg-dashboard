...
const { batchGet, valuesUpdate, valuesAppend } = require('./gwsClient');
...
function savePhoto(eventId, dataUrl) {
  ...
  fs.writeFileSync(path.join(PHOTO_DIR, filename), Buffer.from(match[2], 'base64'));
...
async function clockIn({ opsId, spgName, poiId, poiName, note, lat, lng, accuracy, deviceTime, photoDataUrl }) {
  ...
  const photoRef = savePhoto(eventId, photoDataUrl);
