...
/* The calendar date a session belongs to.
   ...
   This used to read the host's own clock, which is right on a laptop in Indonesia and wrong
   everywhere else: a serverless instance runs UTC, so an SPG clocking in at 06:30 WIB would
   file it under the previous day and get a session id nobody could match to their morning.
   lib/timezone.js pins it to one Indonesian zone instead of the host's. */
function localDateStr(d = new Date()) {
  return tz.dateStr(d);
}

function addDaysLocalStr(d, days) {
  return tz.addDays(d, days);
}
...
async function getToday(opsId) {
  ...
  return {
    ...
    // What the browser needs to lock the clock-out button and count down to the same moment
    // clockOut() will start accepting.
...
/* When a session's clock-out unlocks, and how long is left.

   Exported because the browser has to show the same moment the server enforces. If the two
   computed it separately they would drift — and the visible failure would be a button that
   turns on a minute before the write it triggers is accepted, which reads to an SPG as the
   app being broken rather than as a rule being applied. */
function shiftGate(clockInIso, now = new Date()) {
  ...
  const unlocksAt = new Date(start.getTime() + config.rules.minShiftHours * MS_PER_HOUR);
...
async function clockIn({ opsId, spgName, poiId, poiName, note, lat, lng, accuracy, deviceTime, photoDataUrl }) {
  ...
  // No lateness any more: a shift is judged by its length, not by when it started. See
  // config.rules.minShiftHours.
  ...
  const sessionRow = [
    ...
    '', '', '', '', '',
    needsReview ? 'Needs Review' : 'Clocked In',
...
async function clockOut({ opsId, poiId, poiName, note, lat, lng, accuracy, deviceTime, photoDataUrl, activityResult, activityNote }) {
  ...
  /* The shift-length gate, enforced here and not only in the UI. public/app.js keeps the
     button locked and counts down to the same moment, but a button is a suggestion: the
     record is what this function writes, so this is where the rule has to actually hold. */
  const gate = shiftGate(existing[5], now);
  if (!gate.unlocked) {
    // Says how much longer rather than at what o'clock: the server has no idea which of
    // Indonesia's three zones this SPG reads their phone in, and a wall-clock time in the
    // wrong one is worse than no time at all. The browser, which does know, shows the hour.
    throw Object.assign(
