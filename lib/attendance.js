...
const FLAG = {
  ...
  // Clock-out before the shift was full. NOT yet in ATTENDANCE_FLAG_DETAILS on the dashboard —
  // until it is added there it renders as "Kode tidak dikenal". See INTEGRATION_CONTRACT.md §10.
  SHORT_SHIFT: 'SHORT_SHIFT',
};

// The shortest reason worth storing. Four characters is "ok." with padding; six forces at
// least a word, which is the difference between a reason and a keystroke to get past a form.
...
/* ---------- clock out ---------- */
async function clockOut({ opsId, poiId, poiName, note, lat, lng, accuracy, deviceTime, photoDataUrl, activityResult, activityNote, shortShiftReason, txnId: rawTxn, now: injectedNow }) {
  const txnId = normaliseTxnId(rawTxn);
  ...
  /* Nine hours is a norm now, not a wall.

     It used to be a hard refusal, and refusing was the wrong tool. A shift genuinely does end
     early sometimes — the location closed, the SPG was sent home, something happened — and a
     system that simply says no does not make those days disappear. It makes them go
     unrecorded, which is strictly worse: the day still happened, and now nobody can see it or
     ask about it.

     So an early clock-out is accepted, flagged, and must carry a reason. The SPG is told
     plainly that it will be flagged before they write anything, the reason travels with the
     event where the CF reads it, and the decision about whether it was acceptable belongs to
     the human who can actually judge it. */
  const gate = shiftGate(existing[5], now);
  const shortShift = !gate.unlocked;
  const reason = String(shortShiftReason || '').trim();

  if (shortShift && reason.length < MIN_REASON_LENGTH) {
    // Enforced here and not only in the form. The button is a suggestion; this is the record.
    const worked = formatWorked(now.getTime() - new Date(existing[5]).getTime());
    throw Object.assign(
      new Error(`Absen pulang sebelum ${config.rules.minShiftHours} jam boleh, tapi alasannya wajib diisi. `
        + `Baru ${worked} sejak absen masuk.`),
      { status: 400, gate, needsReason: true },
    );
  }
  ...
  const { distance, flags: locationFlags } = await locationCheck(opsId, poiId, lat, lng);
  const flags = shortShift ? [...locationFlags, FLAG.SHORT_SHIFT] : locationFlags;
  const expiry = addDaysLocalStr(now, config.rules.photoRetentionDays);

  /* The reason goes into the event's SPG Note, which the dashboard already reads (column O)
     and shows in its evidence drawer. A column of its own would have been tidier and would
     have been invisible: the board reads A2:P, so anything past that is a note nobody sees.
     Prefixed rather than replacing an existing note, because a clock-out can be both at a
     hand-typed location and early, and losing either half would lose the reason for a flag. */
  const noteParts = [];
  if (note && String(note).trim()) noteParts.push(String(note).trim());
  if (shortShift) {
    const worked = formatWorked(now.getTime() - new Date(existing[5]).getTime());
    noteParts.push(`Pulang lebih awal (baru ${worked} dari ${config.rules.minShiftHours} jam): ${reason}`);
  }
  ...
  const evRow = eventRow({
    ...
    lat, lng, accuracy, distance, deviceTime, now, photoRef, expiry,
    note: eventNote, flags, txnId,
  });
  await appendEvent(evRow);
  ...
  return { sessionId, eventId, distance, flags: allFlags, shortShift, replayed: false };
}
...
module.exports = {
  ...
  shiftGate, formatLeft, formatWorked, normaliseTxnId, MIN_REASON_LENGTH,
