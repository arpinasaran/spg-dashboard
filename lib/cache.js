...
function createCache({ key, ttlMs, loader, persist: persistable = true }) {
  ...
  function hydrate() {
    if (!hydrated) {
      hydrated = (persistable ? store.readJson(key) : Promise.resolve(null))
  ...
  /* persistable:false means this dataset has no durable snapshot, only memory.

     The snapshot store exists for data that is slow to read and owned by somebody else — the
     184k-row onboarding pipeline, the POI master, the roster. Attendance is neither: it is
     this app's own spreadsheet, it is small, and above all it is WRITTEN by several instances
     at once. Round-tripping it through a single shared file means every instance overwrites
     the file with its own partial view, so a row another instance just wrote disappears from
     the snapshot that the next reader is served. Memory-only is not a downgrade here; it is
     the difference between a cache and a corrupted second source of truth. */
  function persist() {
  ...
  async function loadCold() {
    try {
      ...
      if (persistable && store.storeName() === 'drive') {
        throw Object.assign(
...
function cache({ key, ttlMs, loader, persist }) {
  if (!registry.has(key)) registry.set(key, createCache({ key, ttlMs, loader, persist }));
