...
const HEADER = ['OS ID', 'Nama', 'Password Hash', 'Diperbarui', 'Login Terakhir'];

const PASSWORD_TAB = config.sheets.passwordsTab;
const PASSWORD_RANGE = `'${PASSWORD_TAB}'!A2:D`;
const PASSWORD_HEADER = ['OS ID', 'Nama', 'Kata Sandi', 'Dibuat'];

/* Passwords an SPG can actually type on a phone: one letter and three digits, the digits
   being three consecutive digits taken from their own OS ID.

   This is four characters, and it is worth being plain about what that costs. The search
   space is 24 letters times the handful of three-digit windows an OS ID contains — 96 guesses
   for "OS212341", whose six digits yield four windows. That is small. Two things keep it from
   being worthless:
     ...
     - Repeated failures lock the OS ID for a while (see throttle below), so those 96 guesses
...
function generatePassword(opsId) {
  ...
  if (!windows.length) {
    throw Object.assign(
      new Error(`OS ID "${opsId}" tidak punya 3 angka berurutan untuk dijadikan kata sandi.`),
      { status: 400 },
    );
  }
...
/* Failed-attempt throttle. Under a hundred possibilities is seconds of scripted guessing;
   ...
   In memory, per OS ID. On a serverless instance that means the counter resets whenever a new
...
async function setPassword(opsId, name, password) {
  ...
  if (!id) throw Object.assign(new Error('OS ID wajib diisi.'), { status: 400 });
...
let ensuredPasswords = null;
async function ensurePasswordTab() {
  if (!ensuredPasswords) {
    ensuredPasswords = (async () => {
      const tabs = await listTabs(config.sheets.attendanceDb);
      if (tabs.some(t => t.title === PASSWORD_TAB)) return;
      await batchUpdate(config.sheets.attendanceDb, [{
        addSheet: {
          properties: {
            title: PASSWORD_TAB,
      ...
      await valuesUpdate(config.sheets.attendanceDb, `'${PASSWORD_TAB}'!A1:D1`, [PASSWORD_HEADER]);
    })().catch((err) => { ensuredPasswords = null; throw err; });
  }
  return ensuredPasswords;
}

async function recordPassword(opsId, name, password, when = new Date().toISOString()) {
  await ensurePasswordTab();
  const raw = await readRange(config.sheets.attendanceDb, PASSWORD_RANGE);
  const idx = raw.findIndex(r => String(r[0] || '').trim().toUpperCase() === opsId.toUpperCase());
  const row = [opsId, name || '', password, when];
  if (idx >= 0) {
    await valuesUpdate(config.sheets.attendanceDb, `'${PASSWORD_TAB}'!A${idx + 2}:D${idx + 2}`, [row]);
  } else {
    await valuesAppend(config.sheets.attendanceDb, PASSWORD_RANGE, [row]);
  }
}
...
module.exports = {
  TAB, HEADER, RANGE, PASSWORD_TAB, PASSWORD_HEADER,
