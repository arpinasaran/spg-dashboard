...
const HEADER = ['Ops ID', 'Nama', 'Password Hash', 'Diperbarui', 'Login Terakhir'];

const PASSWORD_TAB = config.sheets.passwordsTab;
const PASSWORD_RANGE = `'${PASSWORD_TAB}'!A2:D`;
const PASSWORD_HEADER = ['Ops ID', 'Nama', 'Kata Sandi', 'Dibuat'];

/* Passwords an SPG can actually type on a phone: one letter and three digits, the digits
   being three consecutive digits taken from their own Ops ID.

   This is four characters, and it is worth being plain about what that costs. The search
   space is 24 letters times the handful of three-digit windows an Ops ID contains — 120
   ...
   keep it from being worthless:
     ...
     - Repeated failures lock the Ops ID for a while (see throttle below), so those 96 guesses
...
function generatePassword(opsId) {
  ...
  if (!windows.length) {
    throw Object.assign(
      new Error(`Ops ID "${opsId}" tidak punya 3 angka berurutan untuk dijadikan kata sandi.`),
      { status: 400 },
    );
  }
...
/* Failed-attempt throttle. Under a hundred possibilities is seconds of scripted guessing;
   ...
   In memory, per Ops ID. On a serverless instance that means the counter resets whenever a new
...
async function setPassword(opsId, name, password) {
  ...
  if (!id) throw Object.assign(new Error('Ops ID wajib diisi.'), { status: 400 });
