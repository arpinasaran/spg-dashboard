...
const FALLBACK_NAMES = [
  'Bq. Elin Widyawati', 'Ni Kadek Ayu Lestari', 'Rizal Maulana', 'Siti Nurhaliza Putri',
...
function pickScenario(r) {
  ...
  if (x < 0.33) return SCENARIOS.EARLY_OUT;
...
function generate({ roster, supervisors, poisByHub, days = 14, people = 18, today = new Date() } = {}) {
  ...
  const guaranteed = [
    ...
    SCENARIOS.OFF_POI, SCENARIOS.EARLY_OUT, SCENARIOS.OPEN, SCENARIOS.ABSENT,
  ];
  ...
  staff.forEach((person, pIdx) => {
    ...
    for (const day of dayList) {
      ...
      const inStatus = late ? 'Late' : 'On Time';
      ...
      const outFlags = [];
      if (!open) {
        const early = scenario === SCENARIOS.EARLY_OUT;
        outTime = early
          ? atTime(day, 14, 20 + Math.floor(r() * 90))
          : atTime(day, 16, 15 + Math.floor(r() * 100));
        outStatus = early ? 'Early' : 'Normal';
        if (early) outFlags.push('EARLY_CLOCKOUT');
      ...
      const overall = needsReview
        ? 'Needs Review'
        : (open ? (late ? 'Late' : 'Clocked In') : 'Clocked Out');
