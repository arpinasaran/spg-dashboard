...
/* What `vercel env pull` writes in place of a value it is not allowed to hand back. Variables
   stored as Secret cannot be read again, by design, so pulling them yields this string.

   It is skipped rather than assigned because assigning it is worse than assigning nothing: a
   caller checking whether a credential is present would be satisfied, and the failure would
   surface much later as an unexplained rejection from Google's auth library instead of as the
   plain "belum di-set" the app already knows how to say. */
