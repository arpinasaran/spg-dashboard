...
/* Which day it is has to be decided in the app's own zone rather than the host's, or a UTC
   instance rolls the week over seven hours late and an SPG's Monday morning still counts
   against last week's target.

   The Date objects themselves are then built from local components on purpose: parseSheetDate
   below also builds local Dates out of the sheet's wall-clock strings, and the two only
   compare like for like if they share a frame. */
function currentWeekRange(now = new Date()) {
  const [y, m, d] = require('./timezone').dateStr(now).split('-').map(Number);
...
function remainingDaysInWeek(now, end) {
  // Same reason as currentWeekRange: "today" is a question about a wall clock in Indonesia,
  // not about wherever this process happens to be running.
  const [y, m, d] = require('./timezone').dateStr(now).split('-').map(Number);
...
// for no benefit.
/* The pipeline columns, fetched once.

   Separated from the counting below because the read is the expensive half and it is the
   same read for everybody: the Sheets API has no server-side filter, so asking for one SPG's
   rows means pulling all 184k and discarding the rest. Computing N SPGs used to mean paying
   that N times. scripts/sync-snapshot.js has to do exactly that for every SPG with an
   account, so one fetch now feeds all of them. */
async function fetchPipeline() {
  // Open-ended ranges ("A4:A", no end row) on purpose. A bounded range silently truncates:
  // Raw_Register is 184k rows and an earlier A4:A99999 cut off 46% of it, which made an
  ...
  return { regDate, regSpg, creDate, creSpg, onbDate, onbStatus, onbSpg };
}

// Pure, given the columns: no I/O, so the week-boundary and source-liveness rules can be
// tested against fixtures rather than against whatever the real pipeline holds today.
function computeKpi(cols, fmsId, now = new Date()) {
  const { regDate, regSpg, creDate, creSpg, onbDate, onbStatus, onbSpg } = cols;
...
async function loadKpi(fmsId, now = new Date()) {
  return computeKpi(await fetchPipeline(), fmsId, now);
}

// One read, many SPGs. Returns fmsId -> kpi.
async function loadKpiBatch(fmsIds, now = new Date()) {
  const cols = await fetchPipeline();
  const out = {};
  for (const fmsId of fmsIds) out[fmsId] = computeKpi(cols, fmsId, now);
  return out;
}
...
module.exports = {
  ...
  fetchPipeline, computeKpi, loadKpi, loadKpiBatch,
};
