...
/* Supervisor mapping for the whole roster. This is the seam the migration will cut along:
   today it answers "who supervises this SPG" from the roster; later the same shape comes from
   the existing CF dashboard and nothing above this line has to change.

   It costs nothing, because the roster read already carries the CF columns. It used to fall
   back to a separate "Data PIC SPG" spreadsheet for layouts that did not; that sheet is no
   longer the source of anything, so asking it would be a second set of permissions to keep
   alive for an answer the roster already gave. */
