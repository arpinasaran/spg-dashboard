...
const ACCEPTED = {
  ...
  // are not even the same shape: OS191121 against Ops1624800. The board joins its roster on
  // FMSID, so an app writing OSID produces sessions that match nobody --
  // every row reads as off-roster and the board looks empty to every CF while the writes are
  // in fact landing. FMSID is the agreed side of that, so it is what opsId means
