...
const ACCEPTED = {
  ...
  // read-only check of all 503 live rows found OSID != FMSID on every single one, and they
  // are not even the same shape (OS191121 against Ops1624800).
  //
  // That makes the pair a live hazard rather than a detail. Whichever side the board joins
  // its roster on has to be the side written here, because it compares the two directly: get
  // it wrong and every session reads as off-roster, so the board looks empty to every CF
  // while the writes are in fact landing. Admins never see it -- seesAll skips the check.
  //
  // OSID is the agreed side, and the board is changing to match. See INTEGRATION_CONTRACT.md.
  opsId: ['OSID'],
  // A different column, and not an identifier here: lib/kpi.js joins Raw_Register /
  // Raw_Creation / Raw_Onboarding on it.
  fmsId: ['FMSID'],
