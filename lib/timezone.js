...
/* Midnight at the start of a "YYYY-MM-DD", as an absolute instant, in the day-boundary zone.

   Indonesia observes no daylight saving, so the zone's fixed offset is exact here. A general
   implementation could not take that shortcut — it would have to search for the instant whose
   local date matches — and the offsets in ZONES are kept precisely so this one case can be
   answered without guessing. Used for the clock-out deadline, which is a midnight in
   Indonesia and must not become a midnight wherever the host happens to run. */
function startOfDay(dateString, zoneName = config.timezone.dayBoundaryZone) {
  const zone = ZONES[zoneName] || ZONES.WIB;
  const sign = zone.offset >= 0 ? '+' : '-';
  const hours = String(Math.abs(zone.offset)).padStart(2, '0');
  return new Date(`${dateString}T00:00:00${sign}${hours}:00`);
}
...
module.exports = { ZONES, zoneFor, zoneNameFor, dayZone, dateStr, addDays, startOfDay, clockStr, labelFor };
