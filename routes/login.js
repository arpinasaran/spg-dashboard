...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* No text-transform here. An FMS ID is mixed case -- "Ops1624800" -- so forcing uppercase
...
<body>
  <form method="post" action="/login">
    <h1>Rute Harian</h1>
    <p class="sub">Masuk dengan FMS ID dan kata sandi kamu.</p>

    <label for="opsId">FMS ID</label>
...
router.post('/login', express.urlencoded({ extended: false }), async (req, res) => {
  ...
  if (!authenticated) {
    ...
    return res.status(401).type('html').send(page({
      opsId,
      error: 'FMS ID atau kata sandi salah.',
    }));
  }
